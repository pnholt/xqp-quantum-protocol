...
const EXPECTED_WHOLE_SUPPLY = 10000000000n;
...
async function main() {
  ...
  const acceptedPrograms = new Set(['spl-token', 'spl-token-2022']);
  if (!data || typeof data !== 'object' || !('parsed' in data) || !acceptedPrograms.has(data.program)) {
  ...
  const expectedRawSupply = EXPECTED_WHOLE_SUPPLY * (10n ** BigInt(decimals));
  const actualRawSupply = BigInt(supply.value.amount);
  ...
  const observed = {
    ...
    expected_raw_supply: expectedRawSupply.toString(),
    ...
    assertions: {
      ...
      exact_raw_supply_10b: actualRawSupply === expectedRawSupply,
      decimals_match_supply: info.decimals === decimals,
